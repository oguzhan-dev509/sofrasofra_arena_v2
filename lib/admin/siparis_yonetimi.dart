...
class _SiparisYonetimiState extends State<SiparisYonetimi> {
  ...
  Future<void> _durumGuncelle({
    ...
    try {
      ...
      final bool readyGecisi = yeniDurum == 'ready';
      final bool onTheWayGecisi = yeniDurum == 'on_the_way';
      final bool deliveredGecisi = _teslimEdildiMi(yeniDurum.toLowerCase());

      bool readyIcinAtamaTetikle = false;
      ...
      await _firestore.runTransaction((tx) async {
        ...
        final String eskiStatus = (data['status'] ?? data['durum'] ?? '')
            .toString()
            .trim()
            .toLowerCase();
        ...
        final bool onceTeslimEdilmis = _teslimEdildiMi(eskiStatus);
        final bool readyyeIlkGecis = readyGecisi && eskiStatus != 'ready';
        final bool deliveredaIlkGecis = deliveredGecisi && !onceTeslimEdilmis;
        ...
        if (deliveredaIlkGecis) {
          ...
          if (assignedCourierId.isNotEmpty) {
            ...
            if (courierSnap.exists) {
              ...
              final int mevcutAktifSiparis = _toInt(
                courierData['aktifSiparis'],
                defaultValue: 0,
              );

              final int mevcutToplamTeslimat = _toInt(
                courierData['toplamTeslimat'],
                defaultValue: 0,
              );

              final int yeniAktifSiparis =
                  mevcutAktifSiparis > 0 ? mevcutAktifSiparis - 1 : 0;

              final Map<String, dynamic> courierUpdate = {
                'aktifSiparis': yeniAktifSiparis,
                'toplamTeslimat': mevcutToplamTeslimat + 1,
                'updatedAt': FieldValue.serverTimestamp(),
        ...
        tx.set(orderRef, updateData, SetOptions(merge: true));
        readyIcinAtamaTetikle = readyyeIlkGecis;
      });

      if (readyIcinAtamaTetikle) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content:
              Text('Sipariş durumu güncellendi: ${_durumLabel(yeniDurum)}'),
        ),
      );
    } catch (e) {
      if (!mounted) return;
