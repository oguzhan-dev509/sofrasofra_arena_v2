...
class ChefTableReservationsPage extends StatelessWidget {
  ...
  Future<void> _updateReservationStatus(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Durum g├╝ncellenemedi: $e'),
        ),
      );
    }
  }
  ...
  String _formatPaymentStatus(String paymentStatus) {
    switch (paymentStatus.toLowerCase()) {
      case 'awaiting_payment':
        return '├ûdeme bekleniyor';
      case 'paid':
        return '├ûdeme tamamland─▒';
      case 'not_required':
        return '├ûdeme gerekmiyor';
      default:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('┼Şef Masas─▒ Rezervasyonlar─▒'),
        backgroundColor: Colors.black,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '├ûdeme: ${_formatPaymentStatus(paymentStatus)}',
                        style: const TextStyle(color: Colors.white70),
                      ),
                      const SizedBox(height: 4),
                      Text(
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Olu┼şturulma: ${_formatDate(createdAtTs)}',
                        style: const TextStyle(color: Colors.white70),
                      ),
                      if (guestCount != null) ...[
                        const SizedBox(height: 4),
                        Text(
                          'Ki┼şi say─▒s─▒: $guestCount',
                          style: const TextStyle(color: Colors.white70),
                        ),
                      ],
                      const SizedBox(height: 4),
