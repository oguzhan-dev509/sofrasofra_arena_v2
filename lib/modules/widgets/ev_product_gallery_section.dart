...
class _EvProductGalleryState extends State<EvProductGallery> {
  ...
  List<String> get _images => EvGalleryManager.normalizeImages(
  ...
  void didUpdateWidget(covariant EvProductGallery oldWidget) {
    ...
    final oldImages = EvGalleryManager.normalizeImages(
      images: oldWidget.images,
      fallbackImage: oldWidget.fallbackImage,
    );
    final newImages = _images;

    if (oldImages.join('|') != newImages.join('|')) {
      final safeIndex =
          newImages.isEmpty ? 0 : _currentIndex.clamp(0, newImages.length - 1);

      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted || !_pageController.hasClients || newImages.isEmpty) {
          return;
        }
