...
class _SaticiSiparisPaneliState extends State<SaticiSiparisPaneli> {
  ...
  Future<void> _updateOrderStatus({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Sipariş durumu güncellendi: ${_statusText(newStatus)}',
          ),
        ),
      );
    } catch (e) {
  ...
  Future<void> _kuryeAta(String orderId) async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Kurye atama süreci başlatıldı")),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
